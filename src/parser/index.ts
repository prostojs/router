...
import { EPathSegmentType, TParsedSegment, TParsedSegmentParametric } from '..'
...
class ParametricNodeWithRegex extends BasicNode<TParsedSegmentParametric> {
    constructor(
        options: TBasicNodeOptions<TParsedSegmentParametric>,
        rgNode: BasicNode,
    ) {
        super(options)
    ...
    beforeOnPop(
        data: TPorstoParserCallbackData<TParsedSegmentParametric>,
    ): void {
...
const staticNode = new BasicNode<TParsedSegment>({
    ...
    .mapContent('value', (content) =>
        content.splice(0).join('').replace(/\\:/g, ':'),
    )
    .popsAtEOFSource(true)

const parser = new BasicNode({}).addRecognizes(
    staticNode,
    paramNode,
    wildcardNode,
)
